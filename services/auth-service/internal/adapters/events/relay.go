...
type OutboxRelay struct {
	outboxRepo users.OutboxRepository
	publisher  users.EventPublisher
...
func (r *OutboxRelay) processBatch(ctx context.Context) error {
	...
	for _, event := range events {
		...
		// Routing Key: event_type (e.g., "user.created")
		...
		// Update status in DB
		err = r.outboxRepo.UpdateEventStatus(ctx, tx, event.ID, users.OutboxStatusPublished)
		if err != nil {
